...
func TestClassifyToolProtocolAttempt_ConservativeRecoveryMatrix(t *testing.T) {
	...
	cases := []struct {
		...
		{"structured tool unavailable refusal", base, attemptObservation{Text: "ToolUnavailable: `synthetic_list_projects` is not available in this environment."}, nil, ReasonCapabilityRefusal},
		{"gitlab permission denial", base, attemptObservation{Text: "GitLab returned permission denied for this project."}, nil, ""},
		{"ordinary unavailable data answer", base, attemptObservation{Text: "The requested data is not available in this environment."}, nil, ""},
