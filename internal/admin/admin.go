...
func (h *handler) docsHandler(w http.ResponseWriter, r *http.Request) {
	...
	envVars := []envVarRow{
		...
		{Name: "PII_REDACTION_MODE", Default: "replace", Description: "One of replace / mask / hash / drop / encrypt. mode=hash REQUIRES PII_HASH_KEY; mode=encrypt (or any PII_ENTITY_ACTIONS entry of the form Entity:encrypt) REQUIRES PII_ENCRYPT_KEY. Boot error otherwise.", CurrentValue: "(see startup log)"},
		...
		{Name: "PII_ENCRYPT_KEY", Default: "(unset)", Description: "Operator-supplied secret required when encrypt is active anywhere (global PII_REDACTION_MODE=encrypt OR any PII_ENTITY_ACTIONS entry of the form Entity:encrypt). Any non-empty string; the gateway derives a 32-byte AES-256-GCM key via SHA-256 at boot. Rotating invalidates every prior encrypted token (breaking change for round-tripping clients).", CurrentValue: "(see startup log)"},
