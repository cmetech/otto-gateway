...
function Set-ManagedSecrets {
    ...
    foreach ($key in @('AUTH_TOKEN','PII_HASH_KEY','PII_ENCRYPT_KEY','PRIVACY_ALIAS_KEY','PRIVACY_TRIAGE_TOKEN')) {
        if ($Values.ContainsKey($key) -and -not $seen.ContainsKey($key)) { $updated.Add("${key}=$($Values[$key])") }
    }
...
function Invoke-Init {
    ...
    if ($authOn) { $managedSecrets.AUTH_TOKEN = $authTokenValue }
    Set-ManagedSecrets -FilePath $overridesDestPath -Values $managedSecrets
    ...
    try {
        switch ($action) {
            ...
            'clear' {
                ...
                } elseif (($rest.Count -eq 2 -and $rest[0] -ceq '--all' -and $rest[1] -ceq '--yes') -or
                          ($rest.Count -eq 1 -and $rest[0] -ceq '--all' -and $Yes.IsPresent)) {
