...
param(
    ...
    [Parameter(Position=1, ValueFromRemainingArguments=$true)][string[]]$PrivacyArgs,
    [string]$Pii,
    [string]$HashKey,
    [string]$EncryptKey,
    [string]$Entities,
    ...
    [string]$Out,
    [string]$CoworkerHome,
    # Support defaults match the POSIX collector: 50MB with oldest rotations
    # dropped first. A 180s timeout is generous enough for real-world bundle
    # assembly and short enough that a runaway
...
function Set-OverridesLine {
    ...
    $content = if (-not (Test-Path -LiteralPath $FilePath)) {
    ...
    if (-not $found) {
        foreach ($line in $content) { $updated.Add($line) }
        $updated.Add("${Key}=${Value}")
    }

    $temporary = Join-Path (Split-Path -Parent $FilePath) ('.overrides-' + [guid]::NewGuid().ToString('N') + '.tmp')
    try {
        ...
        Publish-SupportFileAtomically -TemporaryPath $temporary -DestinationPath $FilePath
...
function Protect-ManagedSecretTemporary {
    ...
    & chmod 600 $FilePath
    if ($LASTEXITCODE -ne 0) { throw 'failed to restrict managed-secret temporary permissions' }
}

function Confirm-ManagedSecretProtection {
    param([Parameter(Mandatory)][string]$FilePath)

    if ([System.Environment]::OSVersion.Platform -ne [System.PlatformID]::Win32NT) {
        & chmod 600 $FilePath
        if ($LASTEXITCODE -ne 0) { throw 'failed to verify managed-secret file permissions' }
        return
    }

    $identity = [System.Security.Principal.WindowsIdentity]::GetCurrent()
    $acl = Get-Acl -LiteralPath $FilePath -ErrorAction Stop
    if (-not $acl.AreAccessRulesProtected) { throw 'managed-secret DACL verification failed' }
    $accessSids = @($acl.Access | ForEach-Object {
        $_.IdentityReference.Translate([System.Security.Principal.SecurityIdentifier]).Value
    } | Sort-Object -Unique)
    $allowsCurrent = @($acl.Access | Where-Object {
        $_.AccessControlType -eq [System.Security.AccessControl.AccessControlType]::Allow -and
        $_.IdentityReference.Translate([System.Security.Principal.SecurityIdentifier]).Value -ceq $identity.User.Value
    }).Count -gt 0
    if ($accessSids.Count -ne 1 -or $accessSids[0] -cne $identity.User.Value -or -not $allowsCurrent) {
        throw 'managed-secret DACL verification failed'
    }
}

function Set-ManagedSecrets {
    param(
        [Parameter(Mandatory)][string]$FilePath,
        [Parameter(Mandatory)][hashtable]$Values,
        [string[]]$CommentedKeys = @()
    )
    $commented = @{}
    foreach ($commentedKey in $CommentedKeys) { $commented[$commentedKey] = $true }
    $existingLines = if (-not (Test-Path -LiteralPath $FilePath)) {
        $ts = (Get-Date).ToUniversalTime().ToString('yyyy-MM-ddTHH:mm:ssZ')
        @(
            "# Generated by 'gw init' / 'gw migrate-to-overrides' on $ts",
            '# Operator customizations + secrets. Loaded AFTER .env, so values here WIN.',
    ...
    foreach ($key in @('AUTH_TOKEN','PII_HASH_KEY','PII_ENCRYPT_KEY','PRIVACY_ALIAS_KEY','PRIVACY_TRIAGE_TOKEN')) {
        if ($Values.ContainsKey($key) -and -not $seen.ContainsKey($key)) {
            $prefix = if ($commented.ContainsKey($key)) { '# ' } else { '' }
            $updated.Add("${prefix}${key}=$($Values[$key])")
        }
    }

    $temporary = Join-Path (Split-Path -Parent $FilePath) ('.managed-secrets-' + [guid]::NewGuid().ToString('N') + '.tmp')
    try {
        $stream = [System.IO.File]::Open(
            $temporary,
            [System.IO.FileMode]::CreateNew,
...
function Invoke-Init {
    ...
    if ($Force -and (Test-Path $destPath)) {
        ...
        if ($RegenerateSecrets) {
            Write-Warning "rotating all five managed secrets invalidates active privacy mappings and encrypted in-flight history."
            Write-Warning "Restart Gateway immediately after rotation; mapping loss is irreversible."
        } else {
            Write-Host "(use -RegenerateSecrets to rotate all five managed secrets)"
        }
    }
    ...
    $managedSecrets = @{
        ...
        PII_ENCRYPT_KEY = $encryptKeyValue
        PRIVACY_ALIAS_KEY = $privacyAliasKeyValue
        PRIVACY_TRIAGE_TOKEN = $privacyTriageTokenValue
    }
    $managedSecretComments = if ($authOn) { @() } else { @('AUTH_TOKEN') }
    Set-ManagedSecrets -FilePath $overridesDestPath -Values $managedSecrets -CommentedKeys $managedSecretComments
    ...
    if ($authOn) {
        ...
        Write-Host '  AUTH:          disabled (generated token stored commented in overrides)'
    }
    if ($hashKeyPreserved) {
...
function Show-Env {
    ...
    $keys = @(
        ...
        'POOL_SIZE','KIRO_WORKER_MAX_TURNS','KIRO_WORKER_IDLE_RECYCLE_MS',
        'KIRO_WORKER_IDLE_RECYCLE_MEMORY_MB','MODEL_CATALOG_REFRESH_INTERVAL_SEC','DEBUG'
    )
    foreach ($k in $keys) {
...
function Test-PrivacyScope([string]$Scope) {
...
function Invoke-PrivacyRequest {
    ...
    $token = $env:PRIVACY_TRIAGE_TOKEN
    if (-not $token -or $token -cnotmatch '^[A-Za-z0-9._~+/-]{1,512}$') {
        throw 'privacy: unavailable (triage token missing or invalid)'
    }
...
function Invoke-PrivacyStatus {
    ...
    if (-not $snapshot.PSObject.Properties['privacy'] -or -not $snapshot.privacy) {
        throw 'privacy: unavailable (safe snapshot has no privacy status)'
    }
    $privacy = $snapshot.privacy
    Write-Host 'privacy:'
    Write-Host ("  profile: {0}" -f $privacy.default_profile)
    Write-Host ("  strict: {0}" -f $(if ($privacy.strict_available) { 'available' } else { 'unavailable' }))
    Write-Host ("  triage: {0}" -f $(if ($privacy.triage_enabled) { 'enabled' } else { 'disabled' }))
    if ($privacy.PSObject.Properties['active_scopes']) { Write-Host ("  scopes: {0} active" -f $privacy.active_scopes) }
    if ($privacy.PSObject.Properties['mapping_entries']) { Write-Host ("  mappings: {0} entries" -f $privacy.mapping_entries) }
}

function Get-RawPrivacyArguments {
    # PowerShell binds -Y/-Yes/--Yes to the global Yes switch before dispatch,
    # losing the spelling that the destructive privacy contract must validate.
    # Recover the literal argv tail used by the supported `pwsh -File`/gw.bat
    # launch path so only exact, case-sensitive `--yes` can authorize clear-all.
    $processArgs = @([System.Environment]::GetCommandLineArgs())
    for ($index = 0; $index -lt ($processArgs.Count - 2); $index++) {
        if ($processArgs[$index] -ine '-File') { continue }
        $candidate = $processArgs[$index + 1]
        try {
        ...
        if (-not [string]::Equals($candidate, $current, $comparison)) { continue }
        if ($processArgs[$index + 2] -cne 'privacy') { return @($PrivacyArgs) }
        $firstPrivacyArgument = $index + 3
        if ($firstPrivacyArgument -ge $processArgs.Count) { return @() }
        return @($processArgs[$firstPrivacyArgument..($processArgs.Count - 1)])
    }
    return @($PrivacyArgs)
}
...
function Invoke-Privacy {
    Initialize-Config
    $argsList = @(Get-RawPrivacyArguments)
    $action = if ($argsList.Count -gt 0) { $argsList[0] } else { '' }
    $rest = @()
    ...
    try {
        ...
        switch ($response.Status) {
            ...
            404 { throw 'privacy: disabled or scope unavailable' }
            { $_ -in @(0,503) } { throw 'privacy: unavailable' }
...
function Invoke-Support {
    ...
    try {
        ...
        foreach ($sub in @('env','health','system','tray','logs\gateway','logs\kiro','logs\co-worker')) {
        ...
        $keys = @(
            ...
            'PII_ENABLED_ENTITIES','PII_ENTITY_ACTIONS','PII_HASH_KEY',
            'PII_ENCRYPT_KEY','AUTH_TOKEN','ALLOWED_IPS',
            'AUTH_TRUST_XFF','HTTP_ADDR','KIRO_CMD','KIRO_ARGS','KIRO_CWD','KIRO_CHAT_LOG_FILE',
            'POOL_SIZE','KIRO_WORKER_MAX_TURNS','KIRO_WORKER_IDLE_RECYCLE_MS',
            'KIRO_WORKER_IDLE_RECYCLE_MEMORY_MB','MODEL_CATALOG_REFRESH_INTERVAL_SEC',
            'STREAM_IDLE_TIMEOUT_SEC','DEBUG','CHAT_TRACE',
            'GW_METRICS_REMOTE_WRITE_URL','GW_METRICS_REMOTE_WRITE_USER',
            'GW_METRICS_REMOTE_WRITE_TOKEN','GW_METRICS_REMOTE_WRITE_INTERVAL_SEC',
        ...
        function New-SafeSourceSnapshot {
            param([string]$Source, [string]$Label, [bool]$WarnIfMissing)
            $item = $null
            try {
                $item = Get-Item -LiteralPath $Source -Force -ErrorAction Stop
            } catch {
                if ($WarnIfMissing) { $collectionWarnings.Add("$Label missing") | Out-Null }
                return $null
            }
            if (($item.Attributes -band [System.IO.FileAttributes]::ReparsePoint) -ne 0) {
                $collectionWarnings.Add("$Label rejected: reparse point") | Out-Null
                return $null
            }
            if ($item.PSIsContainer) {
                $collectionWarnings.Add("$Label rejected: not a regular file") | Out-Null
                return $null
            }
            if (-not $safeOpenAvailable) {
                $collectionWarnings.Add("$Label safe-open unavailable") | Out-Null
                return $null
            }
            ...
            try {
                ...
                if ($baseException.Message -match 'reparse point') {
                    ...
                    $collectionWarnings.Add("$Label unreadable during safe-open") | Out-Null
                }
                return $null
            }

            $snapshotState.Sequence = [int]$snapshotState.Sequence + 1
            $snapshotDirectory = Join-Path $staging 'source-snapshots'
            $snapshotPath = Join-Path $snapshotDirectory ("source-{0}" -f $snapshotState.Sequence)
            try {
        ...
        function Copy-RedactedSupportLog {
            param([string]$Source, [string]$RelativePath, [string]$Label, [bool]$WarnIfMissing)
                ...
                $plainRedactionWork = {
                    ...
                    try {
                        ...
                        $reader = [System.IO.StreamReader]::new(
                            [string]$SnapshotPath, $strictUtf8, [bool]$true)
                        $utf8NoBom = New-Object System.Text.UTF8Encoding($false)
                        $writer = New-Object System.IO.StreamWriter(
                            $TemporaryPath, $false, $utf8NoBom)
                        $plainText = $reader.ReadToEnd()
                        foreach ($redactedLine in @($plainText | Invoke-RedactStream)) {
        ...
        function Copy-RedactedSupportGzip {
            param([string]$Source, [string]$RelativePath, [string]$Label)
            ...
            try {
                ...
                $gzipWork = {
                    ...
                    try {
                        ...
                        $strictUtf8 = New-Object System.Text.UTF8Encoding($false, $true)
                        $reader = [System.IO.StreamReader]::new(
                            $decompressor, $strictUtf8, [bool]$true)
                        ...
                        $plainText = $reader.ReadToEnd()
                        foreach ($line in @($plainText | Invoke-RedactStream)) {
        ...
        $resolvedCoworkerHome = if ($CoworkerHome) { $CoworkerHome } elseif ($env:HERMES_HOME) { $env:HERMES_HOME } else { '' }
        $kiroCwd = if ($env:KIRO_CWD) { $env:KIRO_CWD } else { $GwHome }
        $userHome = if ($env:USERPROFILE) { $env:USERPROFILE } else { $env:HOME }
        if ($kiroCwd -eq '~' -and $userHome) {
            $kiroCwd = $userHome
        } elseif (($kiroCwd.StartsWith('~/') -or $kiroCwd.StartsWith('~\')) -and $userHome) {
            $kiroCwd = Join-Path $userHome $kiroCwd.Substring(2)
        }
        $kiroLog = if ($env:KIRO_CHAT_LOG_FILE) { $env:KIRO_CHAT_LOG_FILE } else { Join-Path $GwHome 'logs\kiro-chat.log' }
        if ($env:KIRO_CHAT_LOG_FILE -and -not [System.IO.Path]::IsPathRooted($kiroLog)) {
            $kiroLog = Join-Path $kiroCwd $kiroLog
        }

        $logsDir = Split-Path -Parent $LogFile
        $chatTrace = if ($LogFile.EndsWith('.log', [System.StringComparison]::OrdinalIgnoreCase)) {
        ...
        if (Test-SupportDirectory $logsDir) {
        ...
        $approvedCoworkerLogs = @(
            'agent.log','errors.log','gateway.log','gui.log','desktop.log','mcp-stderr.log',
            'gateway-shutdown-watchdog.log','dashboard-auth.log','container-boot.log','tool_calls.log'
        )
        function Copy-CoworkerDirectory {
        ...
        $coworkerLogs = if ($resolvedCoworkerHome) { Join-Path $resolvedCoworkerHome 'logs' } else { '' }
        if (-not $resolvedCoworkerHome) {
            $collectionWarnings.Add('Co-worker logs unavailable: no -CoworkerHome or HERMES_HOME configured') | Out-Null
        } elseif (-not (Test-SupportDirectory $resolvedCoworkerHome) -or -not (Test-SupportDirectory $coworkerLogs)) {
            $collectionWarnings.Add('Co-worker logs unavailable: resolved home has no regular logs directory') | Out-Null
        } else {
            Copy-CoworkerDirectory $coworkerLogs 'logs\co-worker'
            ...
            if (Test-SupportDirectory $profilesDirectory) {
                foreach ($profile in Get-ChildItem -LiteralPath $profilesDirectory -Directory -Force -ErrorAction SilentlyContinue) {
                    ...
                    if (Test-SupportDirectory $profileLogs) {
                        Copy-CoworkerDirectory $profileLogs ("logs\co-worker\profiles\{0}\logs" -f $profile.Name)
                    }
                }
            }
        }

        # Both optional snapshots are explicit GETs. Capture JSON is archived
        ...
        function Write-SupportManifest {
            ...
            $manifest.Add("Log content is classified by Gateway's shared SecretClassifier.") | Out-Null
            $manifest.Add("Authoritative credential spans are replaced with [REDACTED].") | Out-Null
            $manifest.Add("") | Out-Null
...
Commands:
  init [flags]        First-run setup -- generates five managed secrets,
  ...
  privacy status|scopes|inspect <scope>|clear <scope>|clear --all --yes
                      Inspect and manage loopback-only privacy triage state
  ...
  support             Produce a redacted diagnostic archive under
                      ...
                      -MaxMb N (default 50), -Timeout SEC (default 180),
                      -LogDays D (default 7), -CoworkerHome DIR.
                      See docs/superpowers/specs/2026-07-27-kiro-and-support-observability-design.md.
...
init flags (for the 'init' subcommand):
  ...
  -RegenerateSecrets  on re-init, mint all five managed secrets.
                      Default preserves existing values to avoid invalidating
                      clients, privacy mappings, or encrypted in-flight history.
