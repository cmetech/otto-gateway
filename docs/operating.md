...
Failure posture — precise guarantee: compression never fails a request. Stage 4 performs no I/O, so there is no endpoint-failure path; the hook's panic recovery still guarantees `(nil, nil)`, and an internal panic forwards the request with whatever stages had already completed applied (stages run in place). Observability: `/health/hooks` (config + lifetime counters, including `budget_unmet`), `gw_compress_runs_total` and `gw_compress_tokens_saved_estimate_total` on `/metrics`. Note the two counters answer different questions: `budget_unmet` counts every enabled run that crossed `COMPRESS_TRIGGER_TOKENS` and ended over `COMPRESS_BUDGET_TOKENS` regardless of whether anything shrank, while the `runs`/`gw_compress_runs_total` counter counts only runs that netted an actual token reduction — so `budget_unmet` can legitimately exceed `gw_compress_runs_total` (e.g. a run where every stage no-opped, or shrank nothing, yet still finished over budget).
