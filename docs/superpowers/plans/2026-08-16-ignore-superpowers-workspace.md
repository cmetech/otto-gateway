...
**Goal:** Keep the repository-root `.superpowers/` workspace out of Git status and prevent its runtime and review artifacts from being committed accidentally.

**Architecture:** Add one root-level directory rule `/.superpowers/` to the existing local tool-state section of `.gitignore`. Verify the rule covers both brainstorm and SDD artifacts while leaving tracked `docs/superpowers/` project documentation unaffected.
...
- Produces: A root-only `/.superpowers/` ignore rule that does not match `docs/superpowers/` or nested `.superpowers/` directories.
...
git ls-files --error-unmatch \
  docs/superpowers/specs/2026-08-16-ignore-superpowers-workspace-design.md
! git check-ignore -q --no-index packages/example/.superpowers/session-token
```

Expected: all commands exit 0. No root `.superpowers/` file appears in status,
the nested `.superpowers/` path remains unignored, and the design document
remains unignored and tracked.
