...
### SHIP WITH FOLLOW-UPS — F1 (High) required before the PowerShell/Windows triage CLI is used

**Single most important reason:** the *gateway's* privacy boundary is sound — every locked invariant on the server side holds under adversarial probing (credentials one-way, restoration provenance-gated, zero-byte strict output, scope isolation/lifecycle, triage confinement, no operational-surface leakage). **But the operator tooling is not at parity:** **F1 (High)** — `gw.ps1 privacy …` sends the `Authorization: Bearer <PRIVACY_TRIAGE_TOKEN>` header *and the raw original↔synthetic mapping response* to whatever proxy `HTTP_PROXY`/`http_proxy` names, because `Invoke-RestMethod` is called without `-NoProxy` (`scripts/gw.ps1:1722`), whereas the POSIX wrapper deliberately uses `curl --noproxy '*'` (`scripts/gw:1928`). On a corporate laptop with a proxy exported in the shell profile, an operator running `gw privacy inspect <scope>` during an incident leaks the break-glass token and the mappings to the proxy's logs. Triage is disabled by default and this is the client wrapper (not the gateway boundary), so it does not make the *gateway* unshippable — but it **must be fixed before the Windows/PowerShell triage CLI is relied upon**, and a stricter reviewer could reasonably call this DON'T SHIP.

Secondary blockers/near-blockers: **M1 (Medium)** — on a deliberately weakened gateway (PII disabled *and* `strict` removed from `PRIVACY_REQUEST_PROFILES`) a `strict` request is silently ignored instead of returning `400 privacy_profile_unavailable` (mitigated by the mandatory workflow receipt-absence check). And a **release-gate correction**: the PowerShell support-bundle suite is **RED on the macOS pwsh runtime (537/540)** — three non-privacy *stage-attribution* assertions fail deterministically; this contradicts the phase summary's "available PowerShell suites: PASS" and must be reconciled (native-Windows result is UNAVAILABLE here).

> **Correction to an earlier draft of this review:** a prior version recorded `pwsh tests/scripts/test-support-bundle.ps1` as EXIT 0 and marked invariant 14 PASS. That was wrong — the EXIT 0 was a placeholder written after a duplicated run was killed, not a real observation. The corrected, first-hand result (EXIT 1, three failures) and the F1/F2–F8 shell-parity findings below supersede it. All server-side (Go) invariant conclusions are unchanged.

---

## 3. Findings (severity-sorted)

No Critical. One High (F1). One Medium (M1). Findings F1–F8 and G1 come from the installer/CLI/shell-parity sub-review and were re-verified against source and by first-hand test runs.

| # | Sev | File:line | Invariant | Defect | Concrete failure scenario | Minimal fix |
|---|---|---|---|---|---|---|
| F1 | **High** | `scripts/gw.ps1:1722` (also `:1738`) | 13/14 (triage token & mappings must not be exposed; loopback-peer) | `Invoke-RestMethod` is called **without `-NoProxy`**, so the loopback triage request is routed through `HTTP_PROXY`/`http_proxy` with the bearer token and the raw mapping response. POSIX defends against exactly this (`curl --noproxy '*'`, `gw:1928`; intent stated at `gw:1892-1894`); PowerShell does not. The URL-string loopback check (`gw.ps1:1697-1706`) validates the URI, but the proxy decides the TCP peer. | Operator on a laptop with `HTTP_PROXY` exported (shell profile / `.envrc` / VPN client) runs `gw privacy inspect <scope>` with triage enabled. The `Authorization: Bearer <PRIVACY_TRIAGE_TOKEN>` header and the full original↔synthetic PII mapping land in the proxy's logs (end-to-end reproduced: proxy log shows `auth=Bearer leak-me-…`). Token reuse then grants full triage until rotation. | Add `-NoProxy` (PS7) and neutralize `DefaultWebProxy` on WinPS 5.1 in `Invoke-PrivacyRequest` and `Invoke-PrivacyStatus`; add a parity test mirroring the POSIX `--noproxy` one. |
...
| G1 | Low (gate) | `tests/scripts/test-support-bundle.ps1:2231,2276` | release gate / test integrity | The PowerShell support-bundle release gate is **RED on macOS pwsh (537/540, EXIT 1)**. Three deadline **stage-attribution** assertions fail (`source-snapshot-chunk`, `gateway-rotation-item`, `gateway-version-probe`) — the timeout message names the wrong blocking stage; sibling stages (`kiro-rotation-item`, `coworker-log-item`, `profile-log-item`, `metrics-request`, `capture-request`, `archive-cap-item`) pass the identical assertion. **Non-privacy** — every fail-closed/no-partial/cancellation/redaction assertion passes. Deterministic across two independent runs plus my first-hand re-run. | An operator whose runbook keys off the reported stage name gets a misleading stage during a support-bundle timeout on those three stages. Also: the phase summary claims "available PowerShell suites: PASS", which is false on this runtime. | Fix the stage label emitted for the three stages (or prove it is a macOS-pwsh timing artifact and gate the assertion on native Windows only). **Native-Windows result UNAVAILABLE.** |
| F5 | Low | `scripts/gw:1921-1935` | 8/12 (mappings memory-only; no avoidable persistence) | The raw triage mapping response is written to a mode-`600` temp file with **no `trap`** to remove it on signal (the only trap is the support-bundle staging trap at `gw:2095`). PowerShell keeps the response in memory only. | `Ctrl-C`/`SIGTERM` during `gw privacy inspect <scope>` leaves a file containing raw original↔synthetic PII mappings in `$TMPDIR` indefinitely. | `trap 'rm -f "$response"' RETURN` inside `privacy_request`/`privacy_status`. |
| F2 | Low | `scripts/gw:1981…2007` vs `scripts/gw.ps1:1825-1828` | 14 (behavioral equivalence) | Exit-code divergence: POSIX returns `2` for usage/validation errors and `1` for operation failure; PowerShell collapses all throws to `exit 1`. | A runbook that retries on rc=1 but aborts on rc=2 silently retries a rejected `clear --all` confirmation on Windows while aborting on POSIX. | Give `Invoke-Privacy` a usage/validation path that `exit 2`s. |
| F3 | Low | `scripts/gw.ps1:1724` | 14 (behavioral equivalence) | PowerShell infers HTTP status from the response body and renders an empty list as no output: `Invoke-RestMethod` yields an empty `Object[]`, `ConvertTo-Json` emits nothing. | `gw privacy scopes` on an idle gateway (HTTP 200 `[]`) prints `[]` on POSIX but nothing on PowerShell; `… | ConvertFrom-Json` then errors on Windows. | Use `-StatusCodeVariable`/`Invoke-WebRequest` and switch on the real status; wrap output with `ConvertTo-Json -AsArray`. |
| F4 | Low | `scripts/gw.ps1:1737-1741` | 14 (behavioral equivalence) | `privacy status` collapses every non-200 into "safe snapshot did not respond"; POSIX reports the HTTP code. | With `AUTH_TOKEN` set, a 401 on `/admin/api/snapshot` tells the Windows operator the gateway is unreachable when it is actually rejecting them. | Surface the status code in the PS catch block. |
| F6 | Low | `scripts/gw:432-450`, `scripts/gw.ps1:983-1000` | 15 (no unrequested posture change) | Re-init strips a leading `#` before matching and rewrites `PRIVACY_ALIAS_KEY`/`PRIVACY_TRIAGE_TOKEN` **uncommented** unconditionally (only `AUTH_TOKEN` honors an enable flag). Parity holds; the behavior is the issue. | An operator who commented out `PRIVACY_ALIAS_KEY` to keep strict unavailable finds it re-activated (strict flips to available) after `gw init --force` / any installer upgrade. Direction is toward stronger protection, so Low. | Carry the commented state of the two privacy keys through the rewrite the way `AUTH_TOKEN` does. |
| F7 | Low | `scripts/gw:1897` vs `scripts/gw.ps1:1700-1702` | 14 (behavioral equivalence) | Address validation is laxer on PowerShell (`127.1`, `2130706433`, `[::ffff:127.0.0.1]`, `HTTP://…` accepted where POSIX rejects). Every PS-only acceptance still resolves to loopback, so **not an escape** — but a parity divergence, and it underlines that only a *string* check gates the peer (see F1). | Divergent accept/reject behavior across shells; combined with F1, the string check is the only client-side gate. | Align the PowerShell loopback grammar with the POSIX regex. |
| F8 | Low | `scripts/lib/redact.sh:14`, `scripts/lib/redact.ps1:29`, `scripts/gw.ps1:2124` | 12 (support redaction integrity) | Production wrappers ship env-gated hooks (`GW_SUPPORT_REDACTOR_BIN`, ~15 `GW_SUPPORT_TEST_*`) that can disable support-bundle safety from the environment. Pre-existing, near-LCE. | An attacker with env control (poisoned rc/`.envrc`/CI wrapper) points redaction at `/bin/cat`; the operator ships an unredacted bundle believing it was scrubbed. | Gate the `GW_SUPPORT_TEST_*` family behind a single self-test guard; require `GW_SUPPORT_REDACTOR_BIN` under `$GW_INSTALL_DIR`. |
...
The two wrong-observable reproductions are R1 (F1, High) and R2 (M1, Medium); R3–R6 demonstrate the core boundary holding under attack.

### R1 — F1: PowerShell triage CLI leaks the token + mappings to a proxy
- **Setup:** triage enabled; fixture `GW_HOME`; `GW_ADDR="http://127.0.0.1:39999"`; a logging proxy on `127.0.0.1:39182`; `HTTP_PROXY`/`http_proxy` exported to that proxy.
- **Command (through the shipped wrapper):** `pwsh -NoProfile -File scripts/gw.ps1 privacy scopes`.
- **Path:** `Invoke-PrivacyRequest` builds `@{ Authorization = "Bearer $token" }` and calls `Invoke-RestMethod … -MaximumRedirection 0` with **no `-NoProxy`** (`gw.ps1:1719-1722`); PowerShell honors the ambient proxy even for a loopback URI.
- **Wrong observable:** the proxy log records `GET http://127.0.0.1:39999/admin/api/privacy/scopes auth=Bearer <PRIVACY_TRIAGE_TOKEN>` — the break-glass credential (and, for `inspect`, the raw original↔synthetic mapping body) is disclosed to the proxy operator. The identical POSIX invocation with the same env leaves the proxy log empty (`curl --noproxy '*'`, `gw:1928`) and reports "Gateway did not respond." Confirmed from source; end-to-end proof from the sub-review.

### R2 — M1: strict header silently ignored on a PII-disabled gateway
...
### R3 — Credentials are one-way (verified by disposable probe, then removed)
...
### R4 — Forged reserved markers block strict output (disposable probe)
...
### R5 — Output-only / cross-scope reserved alias blocks
...
### R6 — Zero-byte strict release (real-server test, cross-checked in source)
...
| 14 | POSIX & PowerShell secret/privacy ops behaviorally equivalent; never print secrets or expose them in avoidable process arguments/transmission | **FAIL** (PowerShell path; POSIX side PASS) | **POSIX PASS:** triage token enters `curl` only via `--config -` on stdin (never argv), `--noproxy '*'`, no `-L`, case-sensitive `--all --yes`; live `ps` capture shows no token; neither wrapper prints `PRIVACY_ALIAS_KEY`/`PRIVACY_TRIAGE_TOKEN`; upgrade preserves managed secrets. **PowerShell keeps the token out of argv but FAILS the equivalence/no-avoidable-exposure clause: F1** ships it to a configured proxy (`gw.ps1:1722`), plus behavioral divergences **F2/F3/F4/F7**. Support redaction still uses the shared Go classifier on both shells (no fork). Native-Windows DACL/SID/junction/WinPS-5.1 checks **UNAVAILABLE** (§9). |
...
- **No skipped tests / environment early-returns** were observed in the Go full or race runs (`grep SKIP` empty).
- **PowerShell support-bundle release gate is RED on macOS pwsh (G1):** 537/540, three stage-attribution assertions fail deterministically (reproduced first-hand + twice by the sub-review). This contradicts the phase summary's "available PowerShell suites: PASS." The failing assertions are **non-privacy** (stage *name* in a 1-second-deadline timeout message); every fail-closed, no-partial-artifact, cancellation, worker-termination, and shared-classifier-redaction assertion in the same suite passes. Because only 3 of ~9 deadline stages fail (siblings pass the identical check), it is either a real per-stage attribution bug for those three stages or a macOS-pwsh timing artifact; **native-Windows behavior is UNAVAILABLE** to disambiguate. Either way the summary's PASS claim for this suite is not reproducible here.
- **Shell parity is weaker than the Go suite proves:** the shell reviewer's F1–F8 (esp. the F1 proxy-token leak) show the installer/CLI wrappers were reviewed independently rather than as a POSIX↔PowerShell diff, so behavioral divergences (exit codes, empty-list rendering, status reporting, proxy handling, address grammar) slipped through with all suites still "passing" on the paths they exercise.
- **Test isolation confirmed:** every shell/pwsh suite uses disposable temp roots (`mktemp -d`/`GetTempPath()`+GUID, hostile `HOME`) and never touches the operator's real home.
...
- **Triage confinement (server side):** disabled-by-default (routes unregistered → 404), actual-peer loopback (proxy headers untrusted), constant-time bearer, `no-store`, no CORS, encoded-path/method-override bypasses fail, clear-all needs an exact confirmation header. **Caveat:** this covers the gateway *endpoint*; the PowerShell *client* wrapper does not confine the token to loopback in transit — see F1. The shared-classifier support/capture redaction (no shell regex fork) was also verified safe on both shells.
...
| `pwsh tests/install/privacy_secrets_windows_test.ps1` | EXIT 0 (portable pwsh 7.6.4/macOS) — 7 ok, **4 skipped** (DACL/current-SID, continuous-reader → UNAVAILABLE, §9) |
| `pwsh tests/scripts/test-privacy-cli.ps1` | EXIT 0 |
| `pwsh tests/scripts/test-support-redact.ps1` | EXIT 0 (39/39) |
| `pwsh tests/scripts/test-support-bundle.ps1` | **EXIT 1 — 537 passed / 3 failed** (first-hand, reproduced; failures = non-privacy stage attribution, see G1; native-Windows UNAVAILABLE). *Corrects a prior draft's erroneous EXIT 0.* |
...
- Real-Windows DACL / current-SID enforcement on managed-secret and `.env`/`overrides.env` files (4 skips in `privacy_secrets_windows_test.ps1`).
- Continuous-reader-during-replacement atomicity on native Windows filesystem semantics.
- Support-bundle / secret-file **junction-swap** and ancestor-swap defenses on NTFS (1 skip in the PS bundle suite).
- **WinPS 5.1 proxy and redirect semantics on the `gw.bat` path** (`gw.bat:9` launches `powershell`, not `pwsh`). `System.Net`'s `BypassProxyOnLocal` matches only dotless hosts, so `127.0.0.1` is *not* bypassed by default — meaning **F1 is likely worse on the primary Windows path**, but this is UNPROVEN here.
- Whether the three **G1** stage-attribution failures reproduce on native Windows (they are deterministic on macOS pwsh; the phase summary implies they pass on the CI Windows runner, which I could not verify).

The portable PowerShell suites ran on macOS pwsh 7.6.4: secret lifecycle, CLI parity, and support-redaction **passed**; the **support-bundle suite failed 3/540 (G1)**.
...
1. **F1 (High — required before the Windows/PowerShell triage CLI is used):** in `Invoke-PrivacyRequest` **and** `Invoke-PrivacyStatus`, neutralize the proxy — add `-NoProxy` on PS7 and set `[System.Net.WebRequest]::DefaultWebProxy = New-Object System.Net.WebProxy` on WinPS 5.1 (the primary `gw.bat` path). **Test:** a PowerShell parity test mirroring the POSIX `--noproxy` one — with `HTTP_PROXY` pointed at a capture server, `gw privacy scopes`/`inspect`/`status` never contact the proxy and never emit the token.
2. **M1 (Medium — fix before ship or first follow-up):** in `Service.Before`, resolve and reject the requested profile *before* the `!PIIEnabled` short-circuit, so a strict (or unknown) request against a PII-disabled / hook-absent gateway returns `400 privacy_profile_unavailable` (spec §6.3/§14). **Test:** a `PIIEnabled:false`, strict-absent service returning `CodeProfileUnavailable` for a `strict` header, asserting no worker dispatch.
3. **G1 (release gate):** reconcile the PowerShell support-bundle gate — fix the stage label for `source-snapshot-chunk`/`gateway-rotation-item`/`gateway-version-probe`, or, if it is a macOS-pwsh timing artifact, make the assertion native-Windows-only and **prove the gate passes on the Windows CI runner** (the summary's "PASS" claim is currently unverified here). **Test:** the three assertions pass deterministically on the target runtime.
4. **F5 (Low):** `trap 'rm -f "$response"' RETURN` in `privacy_request`/`privacy_status` so a killed `gw privacy inspect` leaves no raw-mapping temp file.

The remaining Low items are recommended follow-ups (not ship-blockers), each with its regression test:
5. **L1/L3** receipt coverage semantics — give standard aggregation a non-`full` coverage and stop claiming `full` on strict internal-error paths. **Test:** standard aggregated receipt asserts `coverage!="full"`; strict internal-error receipt asserts truthful coverage.
6. **F2/F3/F4/F7** PowerShell parity — usage-vs-failure exit codes (`exit 2` for validation), read the real HTTP status (`-StatusCodeVariable`), render empty lists as `[]`, surface non-200 status in `privacy status`, align the loopback address grammar. **Test:** a POSIX↔PowerShell parity harness diffing exit codes, empty-list output, and status reporting.
7. **F6** carry the commented state of `PRIVACY_ALIAS_KEY`/`PRIVACY_TRIAGE_TOKEN` through re-init like `AUTH_TOKEN`. **Test:** a deliberately commented alias key stays commented after `gw init --force`.
8. **L2** reconcile `USPhone` between `privacyTechnicalPIIEntities` and `categoryForEntity`. **Test:** pin the chosen category and boot accept/reject of `USPhone:pseudonymize`.
9. **L6** compute the triage `operation` label from path shape, not the pre-routing empty URL param. **Test:** denied `DELETE …/scopes/{id}` records `operation="clear"`.
10. **L7** populate `lastErrorCode` from bounded `*Error.Code` in `observeRequestFailure`. **Test:** after an input block, the posture tile reports `privacy_input_blocked`.
11. **L4/L5/L8/F8** low-cost hardening (document unpadded base64url; tighten or document the `{38,}` bare-payload block; explicit standard-mode `pseudonymize`→replace case without WARN; gate the `GW_SUPPORT_TEST_*`/`GW_SUPPORT_REDACTOR_BIN` overrides behind a self-test guard).

---

*Reviewer's bottom line:* the *gateway's* privacy boundary is correctly implemented and adversarially defensible — every server-side invariant holds. The gap is in the **operator tooling**: F1 (High) leaks the triage token and mappings through a configured proxy on the PowerShell CLI, M1 (Medium) drops the profile-error contract on a PII-disabled gateway, and the PowerShell support-bundle release gate is red (G1) on the runtime I have. Fix F1 before the Windows/PowerShell triage CLI is used, close M1, and reconcile G1 on native Windows; the rest are parity/telemetry/semantics polish. Note the correction in §2 to an earlier draft that overstated PowerShell coverage.
