...
- [ ] 03-01-PLAN.md — Foundation + reachability seam: config D-05 (ENABLED_SURFACES widen + validateEnabledSurfaces), server D-01 SurfaceMount refactor + co-mount regression test, openai adapter skeleton (interfaces + RegisterRoutes + decode.go + goleak testmain), .go-arch-lint.yml adapter_openai boundary

**Wave 2** *(blocked on Wave 1)*

- [ ] 03-02-PLAN.md — Thinnest end-to-end slice (SC1 + SC2/Pi): chat/completions wire decode + render + flat SSE emitter + error envelope + handler (stream + non-stream) + golden fixtures + httptest round-trip
...
| 3. OpenAI Surface | 0/4 | Not started | - |
