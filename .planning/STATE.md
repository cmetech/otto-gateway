...
| 260715-i8k | Show ACP_CAPTURE status on admin About page Feature Flags: new "ACP capture" on/off row (SENSITIVE badge like Chat trace) rendered from Deps.AcpCapture != nil; aboutData.AcpCaptureEnabled field + template row + TestAbout_AcpCaptureRow (on/off). Read-only display, no cmd-layer change. gofumpt/vet/admin-tests/build clean. Branch quick/260715-i8k-acp-capture-about-flag off main (post v2.7.0); not pushed/merged | 2026-07-15 | c795b7a | [260715-i8k-add-acp-capture-status-row-to-admin-abou](./quick/260715-i8k-add-acp-capture-status-row-to-admin-abou/) |
| 260715-kre | Per-worker + gateway CPU/memory: new cgo-free internal/procstat (procfs on linux, OpenProcess/GetProcessTimes/psapi on windows, darwin no-op) feeding two consumers. Prometheus gw_worker_cpu_seconds_total{slot} + gw_worker_resident_memory_bytes{slot} via reggw (inherits gateway_id), labelled by slot never PID; metrics.New+workers closure; Pool.WorkerProcs(). Admin UI: snapshot process_*/per-slot cpu-rss-stat_ok via nil-safe ProcSampler + dashboard live tiles & inline-SVG sparklines (client-side CPU% diff, "n/a" when unavailable). Also registered pre-existing-unregistered metrics + new procstat arch-lint components. build+xcompile(linux/win/darwin cgo-free)+test+race+vet+gofumpt+arch-lint clean; live /metrics+/admin verified. Committed on main; not pushed/merged | 2026-07-15 | 65cc825 | [260715-kre-add-per-worker-cpu-memory-prometheus-met](./quick/260715-kre-add-per-worker-cpu-memory-prometheus-met/) |
