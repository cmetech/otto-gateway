...
---
*Last updated: 2026-06-04 — Phase 08.3.2 REVERTED in favor of a prompt-only fix. The fake-kiro-cli / `-Mode {live,fake}` / `make build-fake-kiro` / TEST-01 machinery was unwound; `$script:PIIPrompt` in `test-pii.ps1` (and `PII_PROMPT` in `test-pii.sh`) was rewritten from "Echo each line back to me VERBATIM" (which Claude refused) to a realistic "Help me draft a customer-support reply email..." form that naturally uses the PII without triggering safety refusal. Phase 8.3 remains complete (v1.9.3 shipped 2026-06-03; ACP Prompt() non-blocking refactor; Windows operator regression closed).*
